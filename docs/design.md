...
| 昵称占用检测 | 场上昵称唯一：welcome 带 `taken` 列表供客户端即时提示，JOIN 时服务器再判一次并回 `reject` + 建议名 |
...
5. **昵称在场上唯一**（含 AI 的 bot1/bot2…）。档案本身是按 `IP::昵称` 分开存的，本可以允许重名，
   但屏幕上出现两条同名的蛇没法辨认，所以入场时做了唯一性校验。客户端的即时提示只是体验优化，
   真正拦截在服务器：两个人同时提交同一个名字时，后到的那个会被回 `reject` 并自动改成 `原名2`。
